...
class DDPM(nn.Module):
    ...
    def forward(self, x, c):
        ...
        """
        c = c.unsqueeze(1).repeat(1, x.shape[1], 1).view(-1, c.shape[-1]) 
        ...
        x_t = (
            self.sqrtab.to(c.device)[_ts, None, None, None] * x
            + self.sqrtmab.to(c.device)[_ts, None, None, None] * noise
    ...
    def sample(self, context, num_samples, return_trajectory=False):
        ...
        x = torch.randn(n_sample, *num_samples).to(device)  # x_T ~ N(0, 1), sample initial noise
        ...
        for i in range(self.n_T, 0, -1):
            ...
            z = torch.randn(n_sample, *num_samples).to(device) if i > 1 else 0
