...
class CNNVAELarge(nn.Module):
    def __init__(
        self, 
        in_channels=1,           # Number of input channels (1 for grayscale, 3 for RGB)
        n_feat=384,              # Number of features in hidden layers, similar to ContextUnet
        latent_dim=10,           # Dimension of the condition vector
        vae_latent_dim=256,      # Dimension of the VAE latent space
        image_size=64,           # Image dimensions (assuming square images)
    ):
        super(CNNVAELarge, self).__init__()
        ...
        # Context embedding
        self.context_embed = EmbedFC(latent_dim, 4 * n_feat)
        
        # Encoder
        self.init_conv = ResidualConvBlock(in_channels, n_feat, is_res=True)
        self.down1 = DownBlock(n_feat, n_feat)
        self.down2 = DownBlock(n_feat, 2 * n_feat)
        self.down3 = DownBlock(2 * n_feat, 4 * n_feat)
        
        # Feature dimension at bottleneck
        self.bottleneck_dim_features = 4 * n_feat * (self.bottleneck_size ** 2)
        
        # Fully connected layers for encoder (mu and logvar)
        self.fc_mu = nn.Linear(self.bottleneck_dim_features + 4 * n_feat, vae_latent_dim)
        self.fc_logvar = nn.Linear(self.bottleneck_dim_features + 4 * n_feat, vae_latent_dim)
        
        # Decoder fully connected
        self.fc_decode = nn.Linear(vae_latent_dim + 4 * n_feat, self.bottleneck_dim_features)
        
        # Decoder network
        self.up0_res = ResidualConvBlock(4 * n_feat, 4 * n_feat, is_res=True)
        self.up1 = UpBlock(4 * n_feat, 2 * n_feat)
        self.up2 = UpBlock(2 * n_feat, n_feat)
        self.up3 = UpBlock(n_feat, n_feat)
    ...
    def decode(self, z, c):
        ...
        h = h.view(-1, 4 * self.n_feat, self.bottleneck_size, self.bottleneck_size)
        ...
        # Final output layer
        x_reconstructed = self.out_conv(h)
