...
class NormalDistributionDataset(Dataset):
    ...
    def __init__(
        ...
        if custom_mu is None or custom_var is None:
            self.mu, self.var = self.generate_params(n_sets, n_features)
        else:
            self.mu = custom_mu
            self.var = custom_var
            
        # Generate samples
        self.data = self.sample(self.mu, self.var, n_sets, set_size, n_features)
        
    
    def generate_params(self, n_sets, n_features):
        mu = np.random.randn(n_sets, n_features)
        var = np.random.randn(n_sets, n_features)**2
        return mu.squeeze(), var.squeeze()
    
    def sample(self, mu, var, n_sets, set_size, n_features):
...
class PoissonDistributionDataset(Dataset):
    ...
    def __init__(
        ...
        if custom_rate is None:
            self.rate = self.generate_params(n_sets, n_features, rate_range)
        else:
            self.rate = custom_rate
            
        # Generate samples
        self.data = self.sample(self.rate, n_sets, set_size, n_features)
        
    
    def generate_params(self, n_sets, n_features, rate_range):
        rate = np.random.uniform(rate_range[0], rate_range[1], (n_sets, n_features))
        return rate.squeeze()
    
    def sample(self, rate, n_sets, set_size, n_features):
        if rate.ndim == 1:
...
class MultinomialDistributionDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            n_sets: Number of parameter sets to generate
            set_size: Number of samples per parameter set
            n_features: Dimensionality of each sample (categories)
        ...
        if custom_probs is None:
            self.probs = self.generate_params(n_sets, n_features)
        else:
        ...
        # Generate samples
        self.data = self.sample(self.probs, n_per_multinomial, n_sets, set_size, n_features)
        
    
    def generate_params(self, n_sets, n_features):
        # Generate probability vectors for each set
        probs = np.random.rand(n_sets, n_features)
        probs = probs / np.sum(probs, axis=1, keepdims=True)
        return probs.squeeze()
    
    def sample(self, probs, n_per_multinomial, n_sets, set_size, n_features):
        if probs.ndim == 1:
